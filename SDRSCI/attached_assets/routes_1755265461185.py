...
@app.route('/api/rfi_data')
def rfi_data():
    """API endpoint to get RFI detection data for map visualization"""
    try:
        # Check age verification
        if not check_age_verification():
            return jsonify({'success': False, 'error': 'Age verification required'})
        
        hours = request.args.get('hours', 24, type=int)
        min_power = request.args.get('min_power', -80, type=float)
        
        # Calculate time threshold
        time_threshold = datetime.utcnow() - timedelta(hours=hours)
        
        # Query RFI detections with location data
        detections_query = db.session.query(
            RFIDetection,
            UserSession.location_lat,
            UserSession.location_lon,
            UserSession.location_city,
            UserSession.location_state,
            UserSession.location_country
        ).join(
            Recording, RFIDetection.recording_id == Recording.id
        ).join(
            UserSession, Recording.session_id == UserSession.session_id
        ).filter(
            RFIDetection.detection_timestamp >= time_threshold,
            RFIDetection.power_level >= min_power
        ).all()
        
        # Format data for map visualization
        detections_list = []
        for detection, lat, lon, city, state, country in detections_query:
            detection_data = {
                'id': detection.id,
                ...
                'latitude': lat,
                'longitude': lon,
                'location': f"{city}, {state}" if city and state else (state or country or 'Unknown')
            }
            detections_list.append(detection_data)
        
        # Calculate statistics
        total_detections = len(detections_list)
        avg_power = sum(d['power_level'] for d in detections_list) / total_detections if total_detections > 0 else 0
        frequencies = [d['frequency'] for d in detections_list]
        freq_span = max(frequencies) - min(frequencies) if frequencies else 0
        ...
        logging.error(f"RFI data API error: {str(e)}")
        return jsonify({'success': False, 'error': str(e)})
